 ...
 * Info:rebirth-search-core CustomMemoryIndex.java 2012-7-6 14:30:11 l.xue.nong$$
 */
...
public class CustomMemoryIndex implements Serializable {
	...
	private static <K, V> Map.Entry<K, V>[] sort(HashMap<K, V> map) {
		...
		if (size > 1)
			ArrayUtil.quickSort(entries, termComparator);
		return entries;
	}
	...
	private static final class ArrayIntList implements Serializable {
		...
		public int[] toArray(int stride) {
			...
			if (stride == 1) {
				...
				for (int i = 0, j = 0; j < size; i++, j += stride)
					arr[i] = elements[j];
			}
			return arr;
		}
	...
	private final class MemoryIndexReader extends IndexReader {
		...
		public int docFreq(Term term) {
			...
			if (DEBUG)
				System.err.println("MemoryIndexReader.docFreq: " + term + ", freq:" + freq);
			return freq;
		}
		...
		public TermEnum terms(Term term) {
			if (DEBUG)
				System.err.println("MemoryIndexReader.terms: " + term);
			...
			return new TermEnum() {
				...
				public boolean next() {
					...
					if (srtFldsIdx >= sortedFields.length)
						return false;
					Info info = getInfo(srtFldsIdx);
					if (++srtTermsIdx < info.sortedTerms.length)
						return true;
				...
				public Term term() {
					if (DEBUG)
						System.err.println("TermEnum.term: " + srtTermsIdx);
					if (srtFldsIdx >= sortedFields.length)
						return null;
					Info info = getInfo(srtFldsIdx);
					if (srtTermsIdx >= info.sortedTerms.length)
		...
		public TermPositions termPositions() {
			...
			return new TermPositions() {
				...
				public boolean next() {
					if (DEBUG)
						System.err.println(".next: " + current + ", oldHasNext=" + hasNext);
				...
				public int read(int[] docs, int[] freqs) {
					if (DEBUG)
						System.err.println(".read: " + docs.length);
					if (!hasNext)
						return 0;
					hasNext = false;
					docs[0] = 0;
				...
				public boolean skipTo(int target) {
					if (DEBUG)
						System.err.println(".skipTo: " + target);
					return next();
				}
		...
		public TermFreqVector[] getTermFreqVectors(int docNumber) {
			if (DEBUG)
				System.err.println("MemoryIndexReader.getTermFreqVectors");
		...
		public void getTermFreqVector(int docNumber, String field, TermVectorMapper mapper) throws IOException {
			if (DEBUG)
				System.err.println("MemoryIndexReader.getTermFreqVector");
			...
			for (int i = info.sortedTerms.length; --i >= 0;) {
				...
				org.apache.lucene.index.TermVectorOffsetInfo[] offsets = new org.apache.lucene.index.TermVectorOffsetInfo[size
						/ stride];
				...
				mapper.map(info.sortedTerms[i].getKey(), numPositions(info.sortedTerms[i].getValue()), offsets,
						(info.sortedTerms[i].getValue()).toArray(stride));
			}
		}
		...
		public byte[] norms(String fieldName) {
			...
			if (fieldName != cachedFieldName || sim != cachedSimilarity) {
				...
				if (DEBUG)
					System.err
							.println("MemoryIndexReader.norms: " + fieldName + ":" + n + ":" + norm + ":" + numTokens);
			}
			return norms;
		}
		...
		public void norms(String fieldName, byte[] bytes, int offset) {
			if (DEBUG)
				System.err.println("MemoryIndexReader.norms*: " + fieldName);
	...
	private static final class VM {
		...
		public static int sizeOfHashMap(int len) {
			return sizeOfObject(4 * PTR + 4 * INT) + sizeOfObjectArray(len) + len * sizeOfObject(3 * PTR + INT);
		}
