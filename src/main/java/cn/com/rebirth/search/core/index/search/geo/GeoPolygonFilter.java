 ...
 * Info:rebirth-search-core GeoPolygonFilter.java 2012-7-6 14:30:38 l.xue.nong$$
 */
		...
		private static boolean pointInPolygon(Point[] points, double lat, double lon) {
			...
			for (i = 0; i < points.length; i++) {
				if (points[i].lon < lon && points[j].lon >= lon || points[j].lon < lon && points[i].lon >= lon) {
					if (points[i].lat + (lon - points[i].lon) / (points[j].lon - points[i].lon)
							* (points[j].lat - points[i].lat) < lat) {
						inPoly = !inPoly;
					}
				}
				j = i;
