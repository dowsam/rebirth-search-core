 ...
 * Info:rebirth-search-core MappingMetaData.java 2012-7-6 14:29:21 l.xue.nong$$
 */
...
public class MappingMetaData {
	...
	public static class Id {
		...
		public boolean equals(Object o) {
			...
			if (path != null ? !path.equals(id.path) : id.path != null)
				return false;
			if (!Arrays.equals(pathElements, id.pathElements))
				return false;
	...
	public static class Routing {
		...
		public boolean equals(Object o) {
			...
			if (required != routing.required)
				return false;
			if (path != null ? !path.equals(routing.path) : routing.path != null)
				return false;
			if (!Arrays.equals(pathElements, routing.pathElements))
				return false;
	...
	public static class Timestamp {
		...
		public boolean equals(Object o) {
			...
			if (enabled != timestamp.enabled)
				return false;
			if (dateTimeFormatter != null ? !dateTimeFormatter.equals(timestamp.dateTimeFormatter)
					: timestamp.dateTimeFormatter != null)
				return false;
			if (format != null ? !format.equals(timestamp.format) : timestamp.format != null)
				return false;
			if (path != null ? !path.equals(timestamp.path) : timestamp.path != null)
				return false;
			if (!Arrays.equals(pathElements, timestamp.pathElements))
				return false;
	...
	public MappingMetaData(DocumentMapper docMapper) {
		...
		this.timestamp = new Timestamp(docMapper.timestampFieldMapper().enabled(), docMapper.timestampFieldMapper()
				.path(), docMapper.timestampFieldMapper().dateTimeFormatter().format());
	}
	...
	public MappingMetaData(CompressedString mapping) throws IOException {
		this.source = mapping;
		Map<String, Object> mappingMap = XContentHelper.createParser(mapping.compressed(), 0,
				mapping.compressed().length).mapOrderedAndClose();
		if (mappingMap.size() != 1) {
			throw new RebirthIllegalStateException("Can't derive type from mapping, no root type: " + mapping.string());
		}
	...
	public Map<String, Object> sourceAsMap() throws IOException {
		Map<String, Object> mapping = XContentHelper.convertToMap(source.compressed(), 0, source.compressed().length,
				true).v2();
		if (mapping.size() == 1 && mapping.containsKey(type())) {
	...
	public ParseContext createParseContext(@Nullable String id, @Nullable String routing, @Nullable String timestamp) {
		return new ParseContext(id == null && id().hasPath(), routing == null && routing().hasPath(), timestamp == null
				&& timestamp().hasPath());
	}
	...
	private void innerParse(XContentParser parser, ParseContext context) throws IOException {
		...
		String routingPart = context.routingParsingStillNeeded() ? routing().pathElements()[context.locationRouting]
				: null;
		String timestampPart = context.timestampParsingStillNeeded() ? timestamp().pathElements()[context.locationTimestamp]
				: null;
