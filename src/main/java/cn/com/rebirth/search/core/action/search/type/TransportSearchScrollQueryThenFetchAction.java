 ...
 * Info:rebirth-search-core TransportSearchScrollQueryThenFetchAction.java 2012-7-6 14:29:28 l.xue.nong$$
 */
...
public class TransportSearchScrollQueryThenFetchAction extends AbstractComponent {
	 ...
	 */
	@Inject
	public TransportSearchScrollQueryThenFetchAction(Settings settings, ThreadPool threadPool,
	...
	private class AsyncAction {
		...
		private void executeQueryPhase(final AtomicInteger counter, DiscoveryNode node, final long searchId) {
			searchService.sendExecuteQuery(node, internalScrollSearchRequest(searchId, request),
		...
		private void executeFetchPhase() {
			...
			for (final Map.Entry<SearchShardTarget, ExtTIntArrayList> entry : docIdsToLoad.entrySet()) {
				...
				FetchSearchRequest fetchSearchRequest = new FetchSearchRequest(queryResults.get(shardTarget).id(),
						docIds);
				DiscoveryNode node = nodes.get(shardTarget.nodeId());
				searchService.sendExecuteFetch(node, fetchSearchRequest,
