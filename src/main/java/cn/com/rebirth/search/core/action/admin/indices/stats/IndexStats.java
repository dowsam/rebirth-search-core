 ...
 * Info:rebirth-search-core IndexStats.java 2012-7-6 14:30:46 l.xue.nong$$
 */
...
public class IndexStats implements Iterable<IndexShardStats> {
	...
	public Map<Integer, IndexShardStats> indexShards() {
		...
		for (Map.Entry<Integer, List<ShardStats>> entry : tmpIndexShards.entrySet()) {
			indexShards.put(entry.getKey(), new IndexShardStats(entry.getValue().get(0).shardRouting().shardId(), entry
					.getValue().toArray(new ShardStats[entry.getValue().size()])));
		}
		return indexShards;
	}
